...
class SR_Dataset(Dataset):
    def __init__(self, args, phase):
        ...
        if phase == 'train':
            print('Manga109 is not training dataset!')
        elif phase == 'val' or phase == 'test':
            self.HR_path = os.path.join(args.dir_data, 'Classical_SR_datasets', 'manga109', 'manga109_HR')
            self.LR_path = os.path.join(args.dir_data, 'Classical_SR_datasets', 'manga109', 'manga109_bicLRx%d'%(args.scale))
        ...
        if phase == 'train':
            ...
            for idx in range(len(self.LR_list)):
                LR_png_name = self.HR_list[idx]
                LR_npy_name = LR_png_name.replace('.png', '.npy')
                if not os.path.isfile(os.path.join(self.HR_path, LR_npy_name)):
                    LR = imageio.imread(os.path.join(self.HR_path, LR_png_name))
